...
from seba.evaluator import Evaluator
from seba.optimization import BasicOptimizationStep, EnsembleOptimizer
from seba.sqlite import SebaSnapshot, SqliteStorage
...
def test_construct_updated_optimization_progress(
    tmp_path: Path, evaluator: Callable[..., Evaluator], test_functions: Any
):
    pytest.importorskip("seba.plugins.optimizer.dakota")
    ...
    config = EnOptConfig.parse_obj(seba_config)
    test_functions = test_functions + (lambda controls, context: controls[2],)

    optimization_work_flow = EnsembleOptimizer(
        evaluator(test_functions), [BasicOptimizationStep(config)]
    )
